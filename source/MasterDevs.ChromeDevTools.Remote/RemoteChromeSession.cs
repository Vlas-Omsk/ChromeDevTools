...
namespace MasterDevs.ChromeDevTools.Remote
{
    internal sealed class RemoteChromeSession : IChromeSession
    {
        ...
        public async Task<T> SendAsync<T>(ICommand<T> command, string? sessionId, CancellationToken cancellationToken)
            where T : ICommandResult
        {
            var request = _commandRequestFactory.Create(command, sessionId);
            ...
            return ((CommandResponse<T>)response).Result;
        }

        private async Task<ICommandResponse> SendInternal<T>(CommandRequest<T> request, CancellationToken cancellationToken)
            where T : ICommandResult
        {
            ...
            _responseWaiters.AddOrUpdate(request.Id, responseWaiter, (key, value) => responseWaiter);

            var webSocket = await GetWebSocket();
            ...
            {
                _responseWaiters.TryRemove(request.Id, out _);
            }
        }

        public void Subscribe<T>(Func<IEvent<T>, Task> handler)
        ...
        private async Task<WebSocket> GetWebSocket()
        {
            if (null != _webSocket)
                return _webSocket;
