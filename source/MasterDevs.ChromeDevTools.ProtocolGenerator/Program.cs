...
namespace MasterDevs.ChromeDevTools.ProtocolGenerator
{
    internal class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            {
                if (solutionDirectory.EnumerateFiles().Any(x => x.Name == "MasterDevs.ChromeDevTools.sln"))
                    break;
            ...
            {
                throw new Exception("Solution dirictory not found");
            }
            ...
            foreach (var protocolFile in protocolFiles)
            {
                ...
                var outputDirectory = Path.Combine(solutionDirectory.FullName, @$"MasterDevs.ChromeDevTools.Protocol.{p.Alias}.Version{_Version}");
                ...
                File.Copy(
                    ...
                    Path.Combine(outputDirectory, $"MasterDevs.ChromeDevTools.Protocol.{p.Alias}.Version{_Version}.csproj")
                );
        ...
        private static void WriteMethodConstants(DirectoryInfo domainDirectoryInfo, string ns)
        {
            ...
            sb.AppendLine();
            sb.AppendLine();
            sb.AppendFormat("namespace {0}.{1}.Version{2}", RootNamespace, ns, _Version);
            sb.AppendLine();
            sb.AppendLine("{");
        ...
        private static void WriteEvent(DirectoryInfo domainDirectoryInfo, string ns, string eventName, string description, IEnumerable<Property> parameters, IEnumerable<string> supportedBy)
        {
            ...
            var sb = new StringBuilder();
            sb.AppendLine("using MasterDevs.ChromeDevTools.Protocol;");
            sb.AppendLine("using Newtonsoft.Json;");
            sb.AppendLine("using System.Collections.Generic;");
            sb.AppendLine();
            sb.AppendFormat("namespace {0}.{1}.Version{2}.{3}", RootNamespace, ns, _Version, domainDirectoryInfo.Name);
            sb.AppendLine();
            sb.AppendLine("{");
